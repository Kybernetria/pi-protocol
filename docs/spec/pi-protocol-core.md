...
Pi Protocol is a capability protocol layer that runs on top of Pi and turns protocol-certified Pi packages into interoperable nodes inside one shared runtime network.

The protocol is capability-first. A node exposes typed callable `provides`. A provide MAY be implemented by deterministic code, LLM-backed reasoning, local Pi resources, or nested protocol calls, but the protocol contract stays the same.
...
A certified node MAY depend on a shared protocol SDK as an ordinary package dependency. That shared SDK is implementation reuse, not a separate user-facing installation step.
...
LLM reasoning MAY be used inside a node implementation or for ambiguity that code cannot responsibly resolve, but MUST NOT be the default mechanism for routine protocol operation.
...
### Bootstrap
The tiny extension logic shipped in every certified package that ensures the shared fabric exists and registers the current node. Bootstrap is intended to be thin extension glue that usually calls into a shared protocol SDK rather than embedding the full runtime in every package.
...
- its own implementation strategy for each provide
...
2. Every certified node MUST ship tiny bootstrap logic in its Pi extension entrypoint.
3. Every certified node MUST be installable and usable on its own.
4. Every certified node MUST register with the shared fabric automatically during runtime activation, typically by `session_start` in Pi.
...
The fabric code is shared infrastructure and MAY be supplied by a shared protocol SDK dependency rather than duplicated inside each certified package.

## 9. Equality clarified

"All nodes are equal" means:
...
TypeScript is the preferred default for the shared SDK and generated package templates because the protocol surface is mostly explicit contract shape rather than implicit convention.
